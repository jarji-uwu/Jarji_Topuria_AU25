...
SELECT
    ...
    TO_CHAR(
        COALESCE(pct_previous_period, 0),
        'FM99.99'
    ) || '%' AS "% PREVIOUS PERIOD",

    TO_CHAR(
        CASE
            WHEN pct_previous_period IS NULL
              OR pct_previous_period = 0
            THEN pct_by_channels
            ELSE pct_by_channels - pct_previous_period
        END,
        'FM99.99'
    ) || '%' AS "% DIFF"
...
SELECT
    ...
    -- centered moving average (rounded)
    ROUND(
        CASE
