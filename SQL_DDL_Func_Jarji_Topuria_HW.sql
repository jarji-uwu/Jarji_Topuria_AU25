...
CREATE OR REPLACE FUNCTION public.films_in_stock_by_title(textpart TEXT)
RETURNS TABLE (
    row_num INT,
    title TEXT,
    language TEXT,
...
SELECT
    ...
    COUNT(i.inventory_id) - COUNT(r.inventory_id) AS available_copies
FROM public.film f
JOIN public.language l ON l.language_id = f.language_id
JOIN public.inventory i ON i.film_id = f.film_id
LEFT JOIN rented r ON r.inventory_id = i.inventory_id
WHERE f.title ILIKE textpart
GROUP BY f.title, l.name
HAVING COUNT(i.inventory_id) - COUNT(r.inventory_id) > 0
),
numbered AS (
SELECT (SELECT COUNT(*) FROM per_film curr WHERE curr.title <= prev.title) AS row_num,
	prev.title,
	prev.language,
...
UNION ALL
SELECT 1,
    'No such films available',
    NULL,
    0
WHERE NOT EXISTS (SELECT 1 FROM per_film);
$$;
