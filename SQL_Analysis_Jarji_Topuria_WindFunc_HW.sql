...
--task 2: total sales for photo products in asia in 2000
--first, aggregate sales per product
--joins used to get from products to times table
...
-- Now we can calculate the overall report total and format numbers
SELECT
    prod_name,
    TO_CHAR(amount_sold, 'FM999999990.00') AS amount_sold,
    TO_CHAR(SUM(amount_sold) OVER (), 'FM999999990.00') AS YEAR_SUM
...
-- aand just format total sales and order by month and product category
SELECT
    month,
    prod_category,
    TO_CHAR(total_sales, 'FM999999990.00') AS total_sales
FROM monthly_category_sales
ORDER BY month, prod_category ASC;
